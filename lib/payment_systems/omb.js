...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
        ...
        if (body.hasOwnProperty('error')) {
            ...
            } else if (body.error.message === "failed to get random outs"){
                console.error("Issue making payments, random outs, will try later");
                if(!extraPaymentRound){
                    setTimeout(function(){
                        makePayments();
                    }, global.config.payout.timerRetry * 60 * 1000);
...
//	console.log("Test " + global.support.decimalToCoin(global.config.payout.maxTxnValue));
//      if (row.amount > global.support.decimalToCoin(global.config.payout.maxTxnValue)) {
//        row.amount = global.support.decimalToCoin(global.config.payout.maxTxnValue);
//      }
...
//                if ((payee.amount > threshold) || (row.amount >= global.support.decimalToCoin(global.config.payout.maxTxnValue))) {
                if (payee.amount > threshold) {
                    payee.setFeeAmount();
//                    if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.amount < (global.support.decimalToCoin(global.config.payout.maxTxnValue)) && payee.address.length !== 109) {
...
/*		    } else if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount >= (global.support.decimalToCoin(global.config.payout.maxTxnValue)) && payee.address.length !== 109) {
                        console.log("Adding " + payee.id + " to the list of people to pay (OG > MaxTxn balance).  Payee balance: " + global.support.coinToDecimal(payee.amount));
