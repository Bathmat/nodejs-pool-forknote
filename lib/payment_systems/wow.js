...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
        ...
        if (body.hasOwnProperty('error')) {
            if (body.error.message === "not enough unlocked money"){
            ...
            } else if (body.error.message === "Transaction would be too large.  try /transfer_split."){
                console.error("Issue making payments, transfer_split, will try later");
                if(!extraPaymentRound){
                    setTimeout(function(){
                        makePayments();
                    }, global.config.payout.timerRetry * 60 * 1000);
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                console.log("Successful payment sent to: " + identifier);
        ...
        rows.forEach(function (row) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                ...
                if (payee.amount > threshold) {
                    payee.setFeeAmount();
                    if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.amount < global.support.decimalToCoin(2000) && payee.address.length !== 108) {
                        console.log("Adding " + payee.id + " to the list of people to pay (OG Address).  Payee balance: " + global.support.coinToDecimal(payee.amount));
                        paymentDestinations.push({amount: payee.amount - payee.fee, address: payee.address});
                        totalAmount += payee.amount;
                        payeeList.push(payee);
//                    } else if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length === 108 && payee.amount >= global.support.decimalToCoin(global.config.payout.exchangeMin)) {
		    } else if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount >= global.support.decimalToCoin(2000) && payee.address.length !== 108) {
                        console.log("Adding " + payee.id + " to the list of people to pay (Balance > 2000).  Payee balance: " + global.support.coinToDecimal(payee.amount));
                        payee.makePaymentAsIntegrated();
                    } else if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length === 108 && (payee.amount >= global.support.decimalToCoin(global.config.payout.exchangeMin) || (payee.amount > threshold && threshold !== 0))) {
                ...
                if (roundCount === rows.length && paymentDestinations.length > 0) {
                    while (paymentDestinations.length > 0) {
                        ...
                        paymentQueue.push(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (body.fee && body.fee > .001) {
                                console.log("Made it to the SQL insert for transactions");
                                let totalAmount = 0;
