...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
        ...
        if (paymentDetails.hasOwnProperty('payment_id')) {
            console.log("Payment made to " + paymentDetails.transfers[0].address + " with PaymentID: " + paymentDetails.payment_id + " For: " + global.support.coinToDecimal(paymentDetails.transfers[0].amount) + " XMR with a " + global.support.coinToDecimal(body.result.fee) + " XMR Mining Fee");
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (typeof body.result.transactionHash !== 'undefined') {
                ...
                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    [0, address, paymentID, amount, body.result.transactionHash.match(hexChars)[0], global.config.payout.mixIn, global.config.payout.fee, 1]).then(function (result) {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (typeof body.result.transactionHash !== 'undefined') {
                ...
                global.mysql.query("INSERT INTO transactions (bitcoin, address, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [0, address, amount, body.result.transactionHash.match(hexChars)[0], global.config.payout.mixIn, global.config.payout.fee, 1]).then(function (result) {
                    payee.transactionID = result.insertId;
                    payee.trackPayment();
                });
            } else {
        ...
        rows.forEach(function (row) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                ...
                if (payee.amount > threshold) {
                    payee.setFeeAmount();
                    if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length !== 187) {
                    ...
                    } else if (payee.bitcoin === 0 && payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length === 187 && (payee.amount >= global.support.decimalToCoin(global.config.payout.exchangeMin) || (payee.amount > threshold && threshold !== 0))) {
                    ...
                    while (paymentDestinations.length > 0) {
                        ...
                        paymentQueue.push(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (typeof body.result.transactionHash !== 'undefined') {
                                ...
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                                    [0, null, null, totalAmount, body.result.transactionHash.match(hexChars)[0], global.config.payout.mixIn, global.config.payout.fee, paymentDetails.transfers.length]).then(function (result) {
                                    paymentDetails.transfers.forEach(function (payeeItem) {
